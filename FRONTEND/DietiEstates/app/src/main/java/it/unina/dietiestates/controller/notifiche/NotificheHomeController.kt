...
class NotificheHomeController (private val context: Context) {
    ...
    fun getNotifiche(callback: (Result<List<NotificaConInfo>>) -> Unit){
        ...
        api.getNotifiche(emailAgente).enqueue(object : Callback<MutableList<NotificaConInfo>> {
            override fun onResponse(call: Call<MutableList<NotificaConInfo>>, response: Response<MutableList<NotificaConInfo>>) {
            ...
            override fun onFailure(call: Call<MutableList<NotificaConInfo>>, t: Throwable) {
                Toast.makeText(context, "Errore durante la connessione al server: ${t.message}", Toast.LENGTH_SHORT).show()
            }
        })
    }

    private fun assegnaErrore(response: Response<MutableList<NotificaConInfo>>): String{
